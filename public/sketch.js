...
stars = [];
for (let i = 0; i < 500; i++) {
    stars[i] = {x: width * Math.random(), 
        y: height * Math.random(),
        size: 1.5 * Math.random() + 1
    };
}

spawnList = [ // botttom appears first
...
let playerShips = loadImage("https://cdn.glitch.com/24f47a7d-8c45-4219-8a3e-b599d1b721b1%2FSpaceshipsResized.png?v=1604809404924");
let playerSprites = [
...
function drawShip(x, y, ID) {
    ...
    ctx.drawImage(playerShips, sprite.x, sprite.y, sprite.width, sprite.height, x - sprite.width/2, y - sprite.height/2, sprite.width, sprite.height);
...
function update(dtime) {
    ...
    if (Touch.isDown) {
        if (!hastouched) {
            hastouched = true;
            offset = {x: Touch.startposition.x - player.x, y: Touch.startposition.y - player.y};
        }
    ...
    if (playerReloadTimer > playerReloadDelay) {
        ...
        let bullet = new Bullet(player.x, player.y - playerSprites[player.spriteID].height/2, 0, -200);
        playerBullets.push(bullet);
    }

    // Spawn enemies
    while (spawnList.length != 0 && worldPos >= spawnList[spawnList.length-1].triggerTime) {
    ...
    for (let i = fragments.length - 1; i >= 0; i--) {
        ...
        if (particle.pos.y > height || particle.removeFlag || particle.pos.x < 0 || particle.pos.x > width || particle.pos.y < 0) fragments.splice(i, 1);
    }
    ...
    for (let i = enemyList.length - 1; i >= 0; i--) {
        ...
        if (enemy.definition.health <= 0) {
            for (let j = 0; j < 100; j++) {
                ...
                fragments.push(new Bullet(enemy.x + sprite.width/2, enemy.y + sprite.height/2, 
                    velocity * Math.cos(theta), velocity * Math.sin(theta)));
            }
        }
        if (enemy.y > height || enemy.definition.health <= 0) enemyList.splice(i, 1);
    }
    ...
    for (let i = enemyBullets.length - 1; i >= 0; i--) {
        ...
        let distPlayerSquared = distanceSquared(bullet.pos.x, bullet.pos.y, player.x, player.y);
        if (distPlayerSquared < player.collisionR2) {
            bullet.removeFlag = true;
            player.health -= 1;
        }
        if (bullet.pos.y > height || bullet.removeFlag || bullet.pos.x < 0 || bullet.pos.x > width || bullet.pos.y < 0) enemyBullets.splice(i, 1);
    }
    ...
    for (let i = playerBullets.length - 1; i >= 0; i--) {
        ...
        enemyList.forEach(enemy => {
            let sprite = sprites[enemy.definition.spriteID];
            let distSquared = distanceSquared(bullet.pos.x, bullet.pos.y, 
                enemy.x + sprite.width/2, enemy.y + sprite.height/2);
            if (distSquared < (sprite.width + sprite.height) * (sprite.width + sprite.height)/18) {
                bullet.removeFlag = true;
                enemy.definition.health -= 1;
            }
        });
        
        if (bullet.pos.y > height || bullet.removeFlag || bullet.pos.x < 0 || bullet.pos.x > width || bullet.pos.y < 0) playerBullets.splice(i, 1);
    }
    ...
    if (player.health <= 0 && worldPos < deathMoment) {
        ...
        for (let j = 0; j < 100; j++) {
            let theta = 2 * Math.PI * Math.random();
            let velocity = 200 * Math.random() + 50;
            fragments.push(new Bullet(player.x, player.y, 
                velocity * Math.cos(theta), velocity * Math.sin(theta)));
        }
        player.x = NaN;
    }
...
var lastRender = 0;
setTimeout(window.requestAnimationFrame(loop), 2000);
// function imgLoaded() {
//     window.requestAnimationFrame(loop);
// }